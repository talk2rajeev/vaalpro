...
import { useDeleteCustomerMutation, useGetCustomersByVendorQuery, useGetCustomersQuery } from '@/features/customers/vendorCustomerApi';
...
const VendorCustomerManagementPage = () => {
  ...
  const vendorCustomersQuery = useGetCustomersByVendorQuery(
    { vendorSysId: vendorId ?? '', page, size: pageSize, sortBy: 'customerSysId', sortDir: 'asc' },
    { skip: !hasVendorContext },
  );
  const { data: vendor } = useGetVendorByIdQuery(vendorId ?? '', { skip: !hasVendorContext });
  ...
  return <TooltipProvider><main className="min-h-screen bg-slate-50 p-8 text-slate-900"><section className="w-full">
    {hasVendorContext && <Breadcrumb><BreadcrumbList className="text-xs"><BreadcrumbItem>Platform Admin</BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink asChild><Link to="/system-admin/vendors">Vendors</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink asChild><Link to={`/system-admin/vendors/${vendorId}/detail`}>{vendor?.vendorName ?? vendorId}</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage className="font-bold text-blue-800">Customers</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb>}
    <div className="flex items-start justify-between"><div><p className={`text-sm font-semibold uppercase tracking-wide text-blue-600 ${hasVendorContext ? 'mt-4' : ''}`}>System Administration</p><h1 className="mt-2 text-3xl font-bold">Manage Vendor Customer</h1><p className="mt-3 text-slate-600">Onboard, manage, and configure details of {vendor?.vendorName ? `${vendor.vendorName} customers` : 'vendor customers and corporate entities'}.</p></div><Button className="shrink-0" onClick={() => setIsAdding(true)}><Plus data-icon="inline-start" className="size-4" />Add Customer</Button></div>
